...
def hook_forwards(self, root_module: torch.nn.Module):
    # 保存原始forward函数的字典
    if not hasattr(self, 'original_forwards'):
...
def unhook_forwards(self, root_module: torch.nn.Module):
    """恢复原始的forward函数"""
        ...
        def matsepcalc(x,contexts,pn,divide):
            ...
            for drow in self.split_ratio:
                ...
                for dcell in drow.cols:
                    ...
                    # 计算当前区域的目标大小
                    target_h = int(latent_h * drow.end) - int(latent_h * drow.start)
                    target_w = int(latent_w * dcell.end) - int(latent_w * dcell.start)
