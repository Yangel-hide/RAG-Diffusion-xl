...
from cross_attention import hook_forwards,TOKENS,TOKENSCON, unhook_forwards
...
class RAGDiffusionXLPipeline(
    ...
    def __init__(
        ...
        # hook_forwards(self, self.unet, rp_type)
    ...
    def unhook_forwards(self, root_module):
        """代理到 cross_attention.py 中的 unhook_forwards"""
        unhook_forwards(self, root_module)
    def encode_prompt(
        self,
        prompt: str,
        ...
        if prompt_embeds is None:
            ...
            for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
                ...
                # print(f"regional_prompt_list: {regional_prompt_list}")
                region_num = len(regional_prompt_list)
    ...
    def regional_forward(self, sample, timestep, encoder_hidden_states, padding=16, *args, **kwargs):
        """
        对每个区域使用对应的提示词计算 latent
        """
        dtype = encoder_hidden_states.dtype
        device = encoder_hidden_states.device
        sample = sample.to(dtype)
        batch_size = sample.shape[0] // 2 if self.do_classifier_free_guidance else sample.shape[0]

        # 1. 使用base提示词生成整体latent
        i = 0
        if self.usebase:
            base_start = self.rpt[i][0] * TOKENSCON
            base_end = self.rpt[i][1] * TOKENSCON  # base提示词在开头
            base_hidden_states = encoder_hidden_states[:, base_start:base_end, :]
            i += 1
            ...
            noise_pred = base_pred
        else:
            noise_pred = torch.zeros_like(sample).to(dtype)
        
        # 2. 对每个区域分别计算
        for _, (m_offset, n_offset, m_scale, n_scale) in enumerate(zip(
            self.HB_m_offset_list,
            self.HB_n_offset_list, 
            self.HB_m_scale_list,
            self.HB_n_scale_list
        )):
            # 计算区域边界
            H, W = sample.shape[-2:]
            # print(f"\nImage size: H={H}, W={W}")
            x_start = int(m_offset * W)
            y_start = int(n_offset * H)
            x_end = int((m_offset + m_scale) * W)
            y_end = int((n_offset + n_scale) * H)
            # print(f"region {i}:")
            ...
            if padding:
                ...
                region_sample = sample[..., y_start_pad:y_end_pad, x_start_pad:x_end_pad]
            else:
                region_sample = sample[..., y_start:y_end, x_start:x_end]

            # 获取当前区域的提示词嵌入
            region_start = self.rpt[i][0] * TOKENSCON
            region_end = self.rpt[i][1] * TOKENSCON
            # print(f"region_start: {region_start}, region_end: {region_end}")
            region_hidden_states = encoder_hidden_states[:, region_start:region_end, :]
            # print(f"region_hidden_states shape: {region_hidden_states.shape}")
            
            # 计算当前区域
            # print(f"region_sample shape: {region_sample.shape}")
            region_pred = self.unet(
                region_sample,
                timestep,
                encoder_hidden_states=region_hidden_states,
                *args,
                **kwargs
            )[0]
            # print(f"region_pred shape: {region_pred.shape}")
            ...
            # 将区域预测放回原位置
            # soft blind
            # noise_pred[..., y_start:y_end, x_start:x_end] = noise_pred[..., y_start:y_end, x_start:x_end] *  (1 - self.base_ratio) + region_pred * self.base_ratio
            if padding:
                orig_h = y_end - y_start
                orig_w = x_end - x_start
                pad_h = y_start - y_start_pad
                pad_w = x_start - x_start_pad
                noise_pred[..., y_start:y_end, x_start:x_end] = \
                    region_pred[..., pad_h:pad_h+orig_h, pad_w:pad_w+orig_w]
            else:
                noise_pred[..., y_start:y_end, x_start:x_end] = region_pred 
            # print("-" * 50)
        return noise_pred
    ...
    def rag_regional_info(self, prompts):
        ppl = prompts.split('BREAK')
        # print('ppl',ppl)
        # targets =[p.split(",")[-1] for p in ppl[:]]
        targets =[p for p in ppl[:]]
        ...
        for pp in targets:
            ...
            pp=[p for p in pp if p != ""]
            tokensnum = len(pp)
            rpt.append([padd, tokensnum // TOKENS + 1 + padd])
            rppt.append(tokensnum)
            padd = tokensnum // TOKENS + 1 + padd
        self.rpt = rpt
        self.rrppt = rppt
        # print('rpt:',self.rpt)
        # print('rppt:',self.rrppt)
        # exit()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        # 在调用开始时检查并清理已存在的hooks
        self.unhook_forwards(self.unet)
        ...
        self.HB_replace = HB_replace
        self.HB_prompt_list = HB_prompt_list
        self.HB_m_offset_list = HB_m_offset_list
        self.HB_n_offset_list = HB_n_offset_list
        self.HB_m_scale_list = HB_m_scale_list
        self.HB_n_scale_list = HB_n_scale_list
        self.regional_prompt = base_prompt +' BREAK '+ " BREAK ".join(HB_prompt_list)
        print(self.regional_prompt)
        self.regional_info(self.prompt) 
        self.rag_regional_info(self.regional_prompt)
        ...
        if self.do_classifier_free_guidance:
            ...
            region_prompt_embeds = torch.cat([region_negative_prompt_embeds, region_prompt_embeds], dim=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # prompt_embeds [2, 308, 2048] 308=77*4
                if i == self.HB_replace:
                    hook_forwards(self, self.unet)
                if i < self.HB_replace:
                    noise_pred = self.regional_forward(
                        latent_model_input,
                        t,
